...
bool Welcome()
{
   ...
   PrintFormat("Waiting for response…");
   res = ReceiveLazyPirate(client, msg);
   PrintFormat("Response succeed=%d, size=%d, data=`%s`", res, msg.size(), msg.getData());  
...
void OnTick()
{
   ...
   PrintFormat("OnTick #%d (connected %d)", tick, connected);
	...
	ZmqMsg msg("MT4 Tick #" + IntegerToString(tick));
	PrintFormat("OnTick #%d: Sending message `%s`", tick, msg.getData()); 
	bool res = client.send(msg);
	PrintFormat("OnTick #%d: Send result: %d", tick, res);
	res = ReceiveLazyPirate(client, msg);
